...
drop table if exists Brightness_Data ;
create table Brightness_Data (
  id integer primary key autoincrement,
  SensorID text,
  Date_n_Time text,
  Brightness text
...
def Brightness_Data_Handler(jsonData):
	#Parse Data 
	json_Dict = json.loads(jsonData)
	SensorID = json_Dict['Sensor_ID']
	Data_and_Time = json_Dict['Date']
	...
	#Push into DB Table
	dbObj = DatabaseManager()
	dbObj.add_del_update_db_record("insert into Brightness_Data (SensorID, Date_n_Time, Brightness) values (?,?,?)",[SensorID, Data_and_Time, Brightness])
	del dbObj
	print("Inserted Brightness Data into Database.")
...
def sensor_Data_Handler(Topic, jsonData):
	...
	elif Topic == "Home/BedRoom/1/Brightness":
		Brightness_Data_Handler(jsonData)		
...
if __name__ == "__main__":
	...
	client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1,"VP")
